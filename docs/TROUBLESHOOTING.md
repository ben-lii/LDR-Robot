...
| `MOTOR_DRIVER=pigpio` / `gpiod` fails on laptop | Native GPIO libs only build/run on the Pi                         | Keep `MOTOR_DRIVER=mock` locally. On Trixie use `gpiod` + `libgpiod-dev`; `pigpio` is not in Trixie apt. |
| `MOTOR_DRIVER=gpiod` fails on Pi                 | Missing `rpi-io` / libgpiod / gpio group                          | `sudo apt install -y libgpiod-dev gpiod`; rebuild `rpi-io`; ensure service user is in `gpio`.           |
